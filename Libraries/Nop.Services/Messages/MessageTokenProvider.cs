...
namespace Nop.Services.Messages
{
    ...
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        public virtual async Task AddOrderTokensAsync(IList<Token> tokens, Order order, int languageId, int vendorId = 0)
        {
            ...
            if (order.StorePickupTimeSlotId.HasValue && order.StorePickupTimeSlotId != 0) // Display Pickup time in email 
            {
                var pickupTime = await _orderService.GetPickupTimeSlotDetail(order.StorePickupTimeSlotId.Value);
                tokens.Add(new Token("Order.PickupTime", pickupTime.TimeSlot));
            }
